...
func (r *CustomerRepository) Create(ctx context.Context, c *model.Customer) error {
	query := `INSERT INTO customers (company_name, pic_name, phone, email, address, npwp)
	          VALUES ($1, $2, $3, $4, $5, $6)
	          RETURNING id, created_at`
	return r.db.QueryRowContext(ctx, query,
		c.CompanyName, c.PICName, c.Phone, c.Email, c.Address, c.NPWP,
	).Scan(&c.ID, &c.CreatedAt)
}

func (r *CustomerRepository) FetchAll(ctx context.Context) ([]model.Customer, error) {
	var customers []model.Customer
	query := `SELECT id, company_name, pic_name, phone, email, address, npwp, created_at
	          FROM customers
...
func (r *CustomerRepository) Update(ctx context.Context, id int, c *model.Customer) error {
	...
	_, err := r.db.ExecContext(ctx, query,
		c.CompanyName, c.PICName, c.Phone, c.Email, c.Address, c.NPWP, id,
	)
	return err
}
